...
from rag_core import DEFAULT_CHROMA_DIR, DEFAULT_DOCS_DIR, DEFAULT_EMBEDDING_MODEL, DEFAULT_MODEL, DEFAULT_PROMPT_NAME, SimpleRAG
...
@mcp.tool()
def responder_con_rag(pregunta: str, k: int = 4, prompt_name: str = DEFAULT_PROMPT_NAME) -> dict:
    """Responde una pregunta usando búsqueda vectorial en Chroma + Ollama Cloud, con prompt de Langfuse trazable."""

    result = rag.answer(pregunta, k=k, model=MODEL, prompt_name=prompt_name)
